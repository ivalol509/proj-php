...
<body style="background-color: #2c2537">
	...
	<div class="col-12 pt-5">
		<h1 class="text-white text-center">Предметы</h1>
		<?php 
			$query1 = mysqli_query($con, "SELECT * FROM pred");
			for ($i=0; $i < mysqli_num_rows($query1); $i++) { 
				$stroka1 = $query1->fetch_assoc();
		?>	
				<div class="col-9 bg-light mt-3 pt-4 pb-4 mx-auto d-flex rounded-pill">
					<h2 style="margin-top: 2px;" class="ml-4"><?php echo $stroka1['name'] ?></h2>
					<a href="pred.php?pred_id=<?php echo $stroka1['id'] ?>" class="ml-auto"><button class="btn rounded-pill text-white pt-2 pb-2 pl-5 pr-5" style="background: linear-gradient(135deg, #689ef4, #9561e6); font-size: 20px;">Вперёд</button></a>
				</div>
		<?php	
			}
		?>
